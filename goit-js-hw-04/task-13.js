...
const getUsersWithEyeColor = (users, color) => {
  // твой код
    return users.filter(user => user.eyeColor === color).map(user => `объект ${user.name}`);
};
...
const getInactiveUsers = users => {
  // твой код
    return users.filter(user => user.isActive === false).map(user => `объект ${user.name}`);
};
...
const getUserWithEmail = (users, email) => {
  // твой код
    ...
    return `объект пользователя ${findUser.name}`
};
